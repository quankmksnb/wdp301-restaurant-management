...
function ProductInfoTab({ parentOptions, childOptions, selectedParentId, onParentChange, fileList, onFileChange }) {
    return (
        <div className="space-y-2">
            {/* ── Thông tin cơ bản ── */}
            <SectionHeader title="Thông tin cơ bản" />
            <div className="grid grid-cols-2 gap-4">
                <Form.Item name="productCode" label="Mã hàng hóa" rules={[{ required: true, message: 'Vui lòng nhập mã hàng' }]}>
            ...
            <div className="grid grid-cols-2 gap-4">
                <Form.Item name="parentCategory" label="Loại thực đơn">
                    <Select options={parentOptions} placeholder="-- Chọn loại --" onChange={onParentChange} />
                </Form.Item>
                <Form.Item name="category" label="Nhóm hàng" rules={[{ required: true, message: 'Vui lòng chọn nhóm hàng' }]}>
                    <Select placeholder="-- Lựa chọn --" options={childOptions} disabled={!selectedParentId} />
                </Form.Item>
            </div>
    ...
    useEffect(() => {
        ...
        getMenuItemById(product._id)
            .then(item => {
                ...
                setFileList((item.images ?? []).map((url, i) => ({
                    uid: `-${i}`,
                    name: `image-${i}`,
                    status: 'done',
